...
function rowHtml(job) {
  ...
  const tags = [job.kind, job.quick ? 'quick' : 'full', job.ranked ? 'ranked' : ''].filter(Boolean).join(' · ');
...
export function matchesPage({ user, queue, running, recent, paused }) {
  const body = `
<h1>Matches</h1>
${paused ? '<p class="warn">The queue is paused; nothing new starts until the organizer resumes it.</p>' : ''}
<h2>Running</h2>
${running.length ? `<table>${running.map(rowHtml).join('')}</table>` : '<p class="dim">Nothing running.</p>'}
...
export function matchPage({ user, job, live, resultText, position }) {
  let status;
  if (job.status === 'queued') status = `<p class="pill q">queued</p><p>Position ${position} in the queue. This page does not refresh itself; reload it.</p>`;
  else if (job.status === 'started') {
    const p = live?.progress;
    status = `<p class="pill run">running</p><p>Started ${fmtDate(job.startedAt)}${p ? ` · sim clock ${Math.round(p.clockSec)} s · ${p.calls} calls · ${fmtDuration(p.elapsedMs)} wall` : ''}. Reload for progress.</p>`;
...
<p><a class="btn" href="/play/?replay=/logs/${esc(job.id)}.json">Watch the replay</a> &nbsp; <a href="/logs/${esc(job.id)}.json">log JSON</a></p>
