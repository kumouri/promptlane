...
Everything else is held identical between arms: same model (`qwen3.5:9b`, think:false, as shipped),
same pilots, same 36 scenarios, same parse/retry/priority-guard path (`translator.parse_schema`/
`enforce_absolute_priority`, imported not reimplemented), and the SAME downstream harness/scoring
(`fidelity_harness.run_pilot`/`summarize`/`guard_diagnostics`, imported unchanged). This file adds a
translation entry point that only varies the prompt text; it does not edit `translator.py`,
`fidelity_harness.py`, or `transparency.py`.
...
Run it (one arm, all three pilots, one live run):
    ...
    python tools/jev/ab_prompt_harness.py --live --arm b --run-id 1 --out runs/ab-arm-b-run1.json
"""
...
def translate_pilot_arm(
    ...
    which prompt-builder produces the text sent to the model. Returns `(schema, diagnostics)`;
    `diagnostics` records compile failures and the `guard_`-named-but-not-a-guard failure mode
    (spec §7) so both are countable per arm without re-deriving them from raw text later."""
    prompt_fn = ARM_PROMPTS[arm]
    if ollama_url is None:
        ollama_url = resolve_ollama_url(None)
    diag = {"attempts": 0, "compile_failures": 0, "guard_named_no_action_failures": 0, "guard_nodes_emitted": 0}
    prompt = prompt_fn(pilot_text, instrument, primary_ability, ultimate_ability)
    last_err: Exception | None = None
    for attempt in range(max_attempts):
        diag["attempts"] += 1
        reply = _ollama_generate(ollama_url, model, prompt, timeout=90.0, max_tokens=1800)
        try:
            ...
            last_err = err
            prompt = (
                prompt_fn(pilot_text, instrument, primary_ability, ultimate_ability)
...
def run_arm(arm: str, pilots: list[str], client, ollama_model: str, schemas_out: Path | None) -> dict:
    pilot_reports = []
    per_pilot_diag = {}
    for name in pilots:
        ...
        print(f"[arm {arm}] translating {name}.md ...", file=sys.stderr)
        schema, diag = translate_pilot_arm(pilot_text, f"prompts/pilots/{name}.md", name, primary, ultimate, arm, model=ollama_model)
...
def main(argv=None) -> int:
    ...
    out = run_arm(args.arm, args.pilots, client, args.ollama_model, schemas_out)
